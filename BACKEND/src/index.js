...
app.post('/api/purchases', async (req, res) => {
  try {
    const { usuario_id, cantidad_monedas, metodo_pago, card_last4 } = req.body;
    ...
    const metodoValido = metodo === 'paypal' || metodo === 'tarjeta' ? metodo : null;
    const compra = await models.Compra.create({ usuario_id, moneda_id: pkg.id, cantidad_monedas, precio_total, estado: 'pagado', metodo_pago: metodoValido || undefined });
    const monedas = (user.monedas || 0) + Number(cantidad_monedas || 0);
    await user.update({ monedas });
    res.json({ compra, usuario: user, card_last4: metodoValido === 'tarjeta' ? String(card_last4 || '').slice(-4) : undefined });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.delete('/api/gifts/:id', async (req, res) => {
  ...
  app.post('/api/donations', async (req, res) => {
    ...
    if (!giftId) {
      ...
      const donation = await models.Donacion.create({ usuario_id, stream_id: stream.id, regalo_id: giftId, monedas_gastadas: amountCoins, puntos_ganados, mensaje });
...
app.post('/api/streams/:id/heartbeat', async (req, res) => {
  try {
    ...
    if (source !== 'dashboard') {
      return res.json({ usuario: null, stream });
    }
    ...
    if (viewerId) {
      ...
      if (viewer) {
        ...
        if (!isStreamer) {
          await stream.update({ usuario_id: viewer.id });
        }
        {
      ...
      // Ignorar heartbeats sin viewer_id para evitar incrementos no deseados
    }
  ...
  try {
    ...
    if (!stream) {
      ...
      if (!isNumeric && usuario_id) {
        stream = await models.Stream.create({ usuario_id, categoria_id: null, titulo: 'Chat Virtual', descripcion: String(stream_id), estado: 'en_vivo', fecha_inicio: nowLocal() });
      } else {
        return res.status(404).json({ error: 'stream no encontrado' });
      }
    }
    const message = await models.MensajeChat.create({ stream_id: stream.id, usuario_id, alias, nivel, texto });
    if (usuario_id) {
      const key = String(usuario_id);
      const prev = messageCounters.get(key) || 0;
      const next = prev + 1;
      messageCounters.set(key, next);
      if (next % 10 === 0) {
        const user = await models.Usuario.findByPk(usuario_id);
        if (user) {
          const state = computeLevelState(user.puntos || 0);
          const adv = advanceLevel(user.nivel_actual || 'Bronce', state.remainder, state.pointsToNextLevel);
