...
export async function purchaseCoins(usuarioId: string, cantidad: number, metodo_pago?: 'paypal' | 'tarjeta', card_last4?: string) {
  const res = await fetch(`${API_BASE}/purchases`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ usuario_id: usuarioId, cantidad_monedas: cantidad, metodo_pago: metodo_pago, card_last4 })
  });
...
export async function streamHeartbeat(streamId: string, seconds: number, viewerId?: string) {
  const res = await fetch(`${API_BASE}/streams/${streamId}/heartbeat`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ seconds, viewer_id: viewerId, source: 'dashboard' })
  });
